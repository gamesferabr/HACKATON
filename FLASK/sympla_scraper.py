...
def scrape_articket_page(url):
    ...
    if response.status_code == 200:
        ...
        print(f"Erro ao acessar o evento. Status Code: {response.status_code}")
        return None
...
def get_event_links(search_url):
    ...
    print(f"Status code: {response.status_code}")  # Verifica se a página foi carregada corretamente
    ...
    if response.status_code == 200:
        ...
        print(f"Erro ao acessar a página de busca. Status Code: {response.status_code}")
        return []
...
if __name__ == "__main__":
    ...
    for event_url in event_links:
        print(f"\nScraping do evento: {event_url}\n")
        event_data = scrape_articket_page(event_url)
        if event_data:
            events_data.append(event_data)

    # Exibir os dados de todos os eventos em forma de lista de objetos
    print(events_data)
